...
const consistentCharacterToggleEl = document.getElementById(
	"consistent-character-toggle",
);
const consistentCharacterPanelEl = document.getElementById(
	"consistent-character-panel",
);
...
const downloadSettingsHintEl = document.getElementById(
	"download-settings-hint",
);
const openDownloadSettingsBtn = document.getElementById(
	...
	await Promise.all(
		characterRecords
			...
			.map(async (r) => {
				...
				if (blob)
					characterThumbUrls.set(r.id, URL.createObjectURL(blob));
			}),
	);
...
function renderCharacterList() {
	...
	characterRecords.forEach((record) => {
		...
		removeBtn.addEventListener("click", () =>
			removeCharacterImage(record.id),
		);
...
consistentCharacterToggleEl.addEventListener("change", () => {
	...
	saveSettings({
		consistentCharacterEnabled: consistentCharacterToggleEl.checked,
	});
	if (consistentCharacterToggleEl.checked) refreshCharacterRecords();
});

characterDropzoneEl.addEventListener("click", () =>
	characterFileInputEl.click(),
);
...
function blobToDataUrl(blob) {
	return new Promise((resolve, reject) => {
		...
		reader.onerror = () =>
			reject(reader.error || new Error("Failed to read an image."));
...
async function resolveQueueImages(item) {
	...
	for (const match of item.matchedImages) {
		...
		images.push({
			...
			fileName:
				(record && record.fileName) || `${match.characterName}.png`,
		});
	}
	return images;
}
		...
		chrome.runtime.sendMessage(
			...
			(response) => {
				resolve(
					response || {
						ok: false,
						error: "No response — is a Flow tab open?",
					},
				);
			},
		);
...
function refreshFlowTab() {
	return new Promise((resolve) => {
		chrome.runtime.sendMessage(
			{ target: "background", type: "REFRESH_FLOW_TAB" },
			(response) => {
				resolve(
					response || {
						ok: false,
...
function showBlockingOverlay(reason, message) {
	if (reason === "not-on-flow") {
		...
		blockingMessageEl.textContent =
			message ||
			"The Flow Automation tool only works when you're on a Flow project page.";
		blockingActionEl.textContent = "Navigate to Flow";
		blockingActionEl.hidden = false;
		blockingActionEl.onclick = () =>
			chrome.tabs.create({ url: FLOW_TOOL_URL });
	} else if (reason === "agent-mode") {
		blockingTitleEl.textContent = "Agent Mode Is On";
		blockingMessageEl.textContent =
			"Turn off Flow's Agent mode to use Overflow — this automation isn't built for that interaction model.";
...
async function checkFlowTab() {
	...
	if (!ping.ok) {
		setStatus(
			ping.error || "No Flow tab found — open a Flow project tab.",
			"error",
		);
	...
	if (ping.agentModeOn) {
		setStatus(
			"Agent mode is on — turn it off in Flow to continue.",
			"error",
		);
...
async function runQueue() {
	...
	if (!refresh.ok) {
		setStatus(
			refresh.error || "No Flow tab found — open a Flow project first.",
			"error",
		);
		resetControls();
		return;
	...
	if (!ping.ok) {
		setStatus(
			ping.error || "No Flow tab found — open a Flow project first.",
			"error",
		);
		resetControls();
		return;
	...
	for (let i = 0; i < queue.length; i++) {
		...
		const result = await sendToContent("RUN_PROMPT", {
			text: queue[i].text,
			images,
		});
	...
	if (running) {
		...
		if (hadErrors) {
			setStatus(
				"Queue complete — some prompts failed. Review and try again.",
				"error",
			);
		} else {
...
chrome.runtime.onMessage.addListener((message) => {
	if (message.target !== "panel" || message.type !== "FLOW_FOCUS_CHANGED")
		return;
	if (!running) {
		focusPaused = false;
		return;
	...
	if (focusPaused && !wasFocusPaused) {
		setStatus(
			"Paused — Flow tab isn't focused (avoiding background-tab throttling). Switch back to resume.",
			"idle",
		);
...
startBtn.addEventListener("click", () => {
	...
	const hasUnfinishedWork =
		queue.length > 0 && queue.some((item) => item.status !== "done");
	...
	if (hasUnfinishedWork) {
		...
		queue = lines.map((text) => {
			const match =
				consistentCharacterToggleEl.checked ?
					CharacterMatcher.matchCharactersInText(
						text,
						characterRecords,
					)
				:	{ matched: [] };
